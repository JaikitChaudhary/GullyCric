      ...
      <div className="mx-auto min-h-screen w-full max-w-md">
        <div className="theme-surface-strong min-h-screen rounded-[2rem] border p-5 backdrop-blur-xl">
          <header className="theme-surface sticky top-4 z-10 rounded-[1.75rem] border px-5 py-5 backdrop-blur">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-3 text-xs uppercase tracking-[0.28em] text-orange-200/75">Players</p>
              </div>
              <ThemeToggle />
            </div>
          </header>
          ...
          <main className="mt-6 space-y-5">
            <Card className="p-5">
              <p className="text-xs uppercase tracking-[0.24em] text-orange-300/70">Player Pools</p>
              <h1 className="mt-2 text-2xl font-semibold text-white">Tournament Players</h1>
              <p className="mt-2 text-sm leading-6 text-slate-300">Choose a tournament to manage its player pool.</p>
            </Card>
            ...
            {!loading && !error && tournaments.length === 0 && (
              <div className="theme-card rounded-[1.75rem] border border-dashed p-8 text-center">
                <p className="text-sm text-slate-300">Create a tournament before adding players.</p>
                <Link
                  to="/tournaments/create"
                  className="mt-5 inline-flex items-center justify-center rounded-full bg-gradient-to-r from-orange-500 to-amber-300 px-5 py-3 text-sm font-semibold text-slate-950"
                >
                  Create Tournament
                </Link>
              </div>
              ...
              <div className="grid gap-3">
                {tournaments.map((tournament) => (
                  ...
                  >
                    ...
                    <p className="mt-1 text-sm text-slate-400">{tournament.overs} overs</p>
                  </button>
                ))}
              </div>
            )}
